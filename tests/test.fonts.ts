...
export async function fontTests(
    urlProvider: (testInfo: TestInfo) => string,
    describeName: string,
    fontTests: SvgFontTest[],
    initHook?: (page: Page) => Promise<void>
): Promise<void> {
    test.describe(describeName, () => {
        fontTests.forEach((fontTest) => {
            test(fontTest.name, async ({ page }, testInfo) => {
                ...
                const svgSrcs = await svgLocator.evaluateAll(
                    (elements) => elements.map((element) => (element as HTMLImageElement).getAttribute('src')).filter(Boolean) as string[]
                );
                ...
                for (let i = 0; i < svgSrcs.length; i++) {
                    ...
                    await test.step(await testStepDescription(mediaInfo), async () => {
                        ...
                        const fontFamilies = await svgTestPage.evaluate(
                            ({ innerTextSelector: textSelector, fontExtractorCode }) => {
                                ...
                                return Array.from(elements)
                                    .map((element) => {
                                        ...
                                        return fontFamily
                                            ? fontFamily.split(',').map((s: string) => s.trim().replace(/^["'](.*)["']$/, '$1'))
                                            : [];
                                    })
